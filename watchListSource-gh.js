var watchlistData = {
  "BanffFilmFestival:Holy(un)HolyRiver": {
    source: {
      YouTube: "Y",
    },
    type: {
      zzzOther: "Y",
    },
    category: {
      zzzOther: "Y",
    },
    notes:
      "(USA, 2015, 60 min)\nDirector and Producer: Pete McBride, Jake Norton\nProduction Company: Pete McBride \u0026 Jake Norton\nWinner – Best Adventure and Exploration Film, 2016 Banff Centre Mountain Film and Book Festival\nTraveling from 5,500 meters atop the Gangotri Glacier, a team of three journeys 2,400 kilometers down The Ganges, the most sacred of all of India’s rivers.",
    title: "Banff Film Festival: Holy (un)Holy River",
    watcher: {
      M1: "Y",
      M2: "Y",
    },
    status: "s3.Soon",
  },
  "SevenBridesForSevenBrothers(OLOS)": {
    source: {
      VHS: "Y",
      DVD: "Y",
      Digital: "Y",
    },
    type: {
      zzzOther: "Y",
    },
    category: {
      zzzOther: "Y",
    },
    title: "Seven Brides For Seven Brothers (OLOS)",
    watcher: {
      M1: "Y",
      M2: "Y",
    },
    status: "s7.Watched",
  },
  "Test02-GH": {
    source: {
      VHS: "Y",
      DVD: "Y",
      Digital: "Y",
    },
    type: {
      zzzOther: "Y",
    },
    category: {
      zzzOther: "Y",
    },
    title: "Test02-GH",
    watcher: {
      M1: "Y",
      M2: "Y",
    },
    status: "s1.Current",
  },
};
